Got it 👍 — let’s prepare a **complete solution sheet** for

👉 **Largest Palindrome in a String – Find the Largest Palindromic Substring**
...
* **Time:** `O(n^3)` → `O(n^2)` substrings × `O(n)` check.
* **Space:** `O(1)` (ignoring substring storage).

---

## 2️⃣ Optimized Approach – Expand Around Center
...
public class LongestPalindromeOptimized {
    ...
    public static String longestPalindrome(String s, boolean preferLast) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            // 🔑 Key difference for first vs last occurrence:
            // preferLast = false → first occurrence
            // preferLast = true  → last occurrence
...
* **Time:** `O(n^2)` → Each expansion worst-case O(n), and there are `2n-1` centers.
* **Space:** `O(1)` → only variables for indices.
...
| Manacher’s Algorithm | `O(n)`          | `O(n)`           | Most efficient, but complex to implement. |
...
👉 Do you want me to also add the **Manacher’s Algorithm (O(n))** version here for completeness, or keep it at brute force + optimized only (enough for 95% of interviews)?
