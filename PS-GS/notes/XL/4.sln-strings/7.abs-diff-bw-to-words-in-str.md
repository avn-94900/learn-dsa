...
3. If `w1` found → compute its **midpoint** and compare with last seen `w2` midpoint.
4. If `w2` found → compute its **midpoint** and compare with last seen `w1` midpoint.
...
public class MinDistanceLinear {
    public static int minDistance(String text, String w1, String w2) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i + w1.length() <= n && text.startsWith(w1, i)) {
            ...
            if (i + w2.length() <= n && text.startsWith(w2, i)) {
